...
function Onboarding({onDone}) {
  ...
  return <div style={{minHeight:"100vh",background:C.bg,fontFamily:"'DM Sans',sans-serif",display:"flex",flexDirection:"column"}}>
    ...
    {screen==="welcome" && <div style={{flex:1,display:"flex",flexDirection:"column",justifyContent:"flex-end",padding:24}}>
      ...
      <PrimaryBtn onClick={()=>setScreen("privacy")} style={{marginBottom:10}}>Get Started</PrimaryBtn>
      <button 
        onClick={async()=>{try{await signInWithPopup(auth,googleProvider);}catch(e){alert("Sign-in error: "+e.message); console.log(e);}}}
        style={{width:"100%",background:C.card2,color:C.white,border:`1px solid ${C.border}`,borderRadius:12,padding:"16px 0",fontSize:16,fontWeight:700,display:"flex",alignItems:"center",justifyContent:"center",gap:10}}
      >
      ...
      <div style={{textAlign:"center",marginTop:16,fontSize:12,color:C.muted2}}>Already set up? <span style={{color:C.accent,cursor:"pointer"}} onClick={()=>setScreen("restore")}>Restore with code →</span></div>
    </div>}
    ...
    {screen==="restore" && <div style={{flex:1,display:"flex",flexDirection:"column",padding:24}}>
      ...
      <div style={{fontSize:14,color:C.muted2,marginBottom:32,lineHeight:1.7}}>Paste your backup code below. If you previously used Google, just go back and sign in.</div>
      <textarea 
...
export default function App() {
  ...
  if(!hasProfile && (!uid || (uid && cloudLoaded))) return <Onboarding onDone={p=>{
  ...
  if(uid && !cloudLoaded && !hasProfile) return <div style={{minHeight:"100vh",background:C.bg,display:"flex",alignItems:"center",justifyContent:"center"}}>
    <style>{CSS}</style>
    <div style={{textAlign:"center"}}>
      <div style={{fontSize:32,animation:"spin 1s linear infinite",display:"inline-block",marginBottom:12}}>☁️</div>
      <div style={{fontSize:13,color:C.muted2}}>Syncing your profile...</div>
    </div>
  </div>;
      ...
      {tab==="insight"&&<>
        ...
        <Card>
          ...
          {user?<>
            ...
            <PrimaryBtn onClick={async()=>{try{await signInWithPopup(auth,googleProvider);}catch(e){alert("Sign-in error: "+e.message); console.log(e);}}} style={{width:"100%",display:"flex",alignItems:"center",justifyContent:"center",gap:8}}>
